...
file_path = 'JQ2Jdata_34884157.xlsx'
...
plt.plot(data['Date'], data['turnover'], marker='o', color='red')
plt.title('Wholesale/retail trade; repair mtr vehicles/mtorcycles TOTAL (£m)')
plt.xlabel('Date')
plt.ylabel('turnover')
...
# ### Visual representation of the trend and seasonality components in the 'repair' time series data using moving averages.
...
for i in np.arange(3, len(data) - 3):
    MA7[i] = np.mean(data['turnover'][(i-3):(i+4)])
...
for i in np.arange(6, len(data) - 6):
    MA2x12[i] = np.sum(data['turnover'][(i-6):(i+7)] * np.concatenate([[1/24], np.repeat(1/12, 11), [1/24]]))

# Plot original time series
plt.figure(figsize=(10, 6))
plt.plot(data.index, data['turnover'],color = 'pink', label='Original')
plt.plot(MA7.index, MA7, color = "red",label='Seasonality')
plt.plot(MA2x12.index, MA2x12,color = "green", label='Trend')
plt.title('Trend Estimator')
plt.xlabel('Date')
plt.ylabel('turnover')
plt.legend()
plt.grid(True)
...
decomposition = seasonal_decompose(data['turnover'], model='additive')

# Plot the decomposed components
plt.figure(figsize=(10, 8))
plt.subplot(4, 1, 1)
plt.plot(data.index, data['turnover'], label='Original')
plt.legend()
...
# Read the data
data = pd.read_excel("JQ2Jdata_34884157.xlsx")
...
# Plot scatter plot
plt.figure(figsize=(10, 6))
plt.scatter(data['Year'], data['turnover'], color='blue', alpha=0.5)
plt.title('Wholesale/retail trade; repair mtr vehicles/mtorcycles TOTAL (£m)')
plt.xlabel('Year')
plt.ylabel('K54D')
plt.grid(True)
...
# Plot ACF
plt.figure(figsize=(10, 6))
plot_acf(data['turnover'], lags=50, ax=plt.gca(),  color = "green")
...
fit1 = ExponentialSmoothing(data['turnover'], seasonal='add').fit()

# Fit Holt-Winters model with multiplicative seasonality
fit2 = ExponentialSmoothing(data['turnover'], seasonal='mul').fit()
...
rmse_additive = rmse(data['turnover'], fit1.fittedvalues)

# Calculate RMSE for the multiplicative model
rmse_multiplicative = rmse(data['turnover'], fit2.fittedvalues)
...
forecast = fit1.forecast(len(forecast_index))  # Using the additive model for forecasting
