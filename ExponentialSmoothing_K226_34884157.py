...
file_path = 'K226data_34884157.xlsx'
...
plt.plot(data['Date'], data['production'], marker='o', color='red')
plt.title('Movements in the volume of production Over Time')
plt.xlabel('Date')
...
# ### Visual representation of the trend and seasonality components in the 'production' time series data using moving averages.
...
for i in np.arange(3, len(data) - 3):
    MA7[i] = np.mean(data['production'][(i-3):(i+4)])
...
for i in np.arange(6, len(data) - 6):
    MA2x12[i] = np.sum(data['production'][(i-6):(i+7)] * np.concatenate([[1/24], np.repeat(1/12, 11), [1/24]]))

# Plot original time series
plt.figure(figsize=(10, 6))
plt.plot(data.index, data['production'],color = 'pink', label='Original')
...
plt.subplot(4, 1, 1)
plt.plot(data.index, data['production'], label='Original')
plt.legend()
...
# Read the data
data = pd.read_excel("K226data_34884157.xlsx")
...
# Plot scatter plot
plt.figure(figsize=(10, 6))
plt.scatter(data['Year'], data['production'], color='blue', alpha=0.5)
plt.title('Movements in the volume of production Over Time')
plt.xlabel('Year')
plt.ylabel('production')
plt.grid(True)
...
# Plot ACF
plt.figure(figsize=(10, 6))
plot_acf(data['production'], lags=50, ax=plt.gca(), color = "green")
...
fit1 = ExponentialSmoothing(data['production'], seasonal='add').fit()

# Fit Holt-Winters model with multiplicative seasonality
fit2 = ExponentialSmoothing(data['production'], seasonal='mul').fit()
...
plt.xlabel('Date')
plt.ylabel('production')
plt.title('Movements in the volume of production')
plt.legend()
plt.show()
...
rmse_additive = rmse(data['production'], fit1.fittedvalues)

# Calculate RMSE for the multiplicative model
rmse_multiplicative = rmse(data['production'], fit2.fittedvalues)

print("RMSE for Additive Model:", rmse_additive)
print("RMSE for Multiplicative Model:", rmse_multiplicative)


# ### Holt Linear Holt Linear  exponential smoothing model

# ### The code generates forecasts until December 2024 using the multiplicative Holt Linear  exponential smoothing model and prints the forecasted values in a DataFrame.
...
model = ExponentialSmoothing(data['production'], trend='add', seasonal_periods=12)
fit_model = model.fit()

# Generate forecasts for the existing dates in the Excel file
forecast_existing_dates = fit_model.predict(start=data.index[0], end=data.index[-1])

# Print the forecasted values
print("Forecasted Extraction for existing dates:")
print(forecast_existing_dates)
...
# Calculate RMSE
rmse = np.sqrt(mean_squared_error(data['production'], forecast_existing_dates))
...
plt.figure(figsize=(10, 6))
plt.plot(data.index, data['production'], label='Actual Production', color='orange')
plt.plot(forecast_index, forecast, label='Forecasted Production', color='red')
plt.title('Production Forecast until December 2024')
plt.xlabel('Date')
plt.ylabel('Production')
plt.grid(True)
plt.xticks(rotation=45)
